      ...
      <section className="py-32 bg-secondary-50 font-sans">
        <div className="max-w-5xl mx-auto px-6 md:px-12">
          <motion.div 
            ...
            viewport={{ once: true }}
            className="bg-white rounded-[3.5rem] border border-secondary-100 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] overflow-hidden mb-20 premium-shadow"
          >
             <table className="w-full text-left border-collapse group/table">
               <thead>
                 <tr className="bg-secondary-900 text-white">
                    <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] opacity-60">Structure</th>
                    <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em]">Invoice Factoring</th>
                    <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em]">Supply Chain</th>
                 </tr>
               </thead>
               <tbody className="divide-y divide-secondary-100">
                 {[
                   { label: 'Fee range', v1: '1–5%', v2: '1–5%', type: 'bold' },
                   { label: 'Based on', v1: 'Invoice size + buyer credit', v2: 'PO size + term length', type: 'small' },
                   { label: 'Advance rate', v1: 'Up to 90%', v2: 'Up to 100% of PO', type: 'bold' },
                   { label: 'Term', v1: '1d – 6 months', v2: '1d – 6 months', type: 'bold' },
                   { label: 'Processing fee', v1: 'None', v2: 'None', type: 'accent' },
                   { label: 'Early repayment', v1: 'No penalty', v2: 'No penalty', type: 'caps' },
                 ].map((row, idx) => (
                   <tr key={idx} className="hover:bg-primary-50/30 transition-all duration-300 cursor-default group/row hover:scale-[1.01] origin-center">
                     <td className="px-10 py-8 text-sm font-bold text-secondary-400 group-hover/row:text-primary-600 transition-colors uppercase tracking-widest text-[10px]">{row.label}</td>
                     <td className={cn(
                       "px-10 py-8 transition-all duration-300 group-hover/row:translate-x-1",
                       row.type === 'bold' ? "text-xl font-black text-secondary-900" : 
                       row.type === 'small' ? "text-secondary-600 font-medium" :
                       row.type === 'accent' ? "text-sm font-black text-primary-600" :
                       "text-xs font-black uppercase tracking-[0.2em] text-secondary-900"
                     )}>{row.v1}</td>
                     <td className={cn(
                       "px-10 py-8 transition-all duration-300 group-hover/row:translate-x-1",
                       row.type === 'bold' ? "text-xl font-black text-secondary-900" : 
            ...
            {[
              ...
              <details key={i} className="bg-white border border-secondary-100 rounded-3xl group transition-all">
                <summary className="p-8 cursor-pointer list-none flex items-center justify-between">
                  ...
                  <AnimatedIcon icon={ChevronRight} size={24} className="text-secondary-400 group-open:rotate-90 transition-transform" animation="draw" />
                </summary>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            ...
            <motion.div 
              ...
              viewport={{ once: true }}
              className="bg-secondary-900 rounded-[3rem] p-12 text-white overflow-hidden relative shadow-2xl premium-shadow atmospheric-glow"
            >
               ...
               <div className="relative z-10 space-y-12">
                  <div className="flex items-center gap-4 text-primary-400">
                    <AnimatedIcon icon={Calculator} size={32} trigger="view" animation="float" />
                  ...
                  <div className="pt-8 space-y-6">
                    <div className="flex items-start gap-3 p-4 bg-white/5 rounded-2xl border border-white/5">
                      <AnimatedIcon icon={Info} size={16} className="text-secondary-400 flex-shrink-0 mt-0.5" animation="pulse" />
